...
def compute_approx_kl(
    ...
    retrieve_mask: Optional[torch.Tensor] = None,
    ...
    if retrieve_mask is not None:
        log_ratio = log_ratio * retrieve_mask
...
def compute_reward(
    ...
    if kl_coef <= 0.0:
        kl_coef = 0.0
    # print("kl_coef:", kl_coef)  # Disabled because it is too noisy.
    # kill
...
def compute_reward_new(
    ...
    if action_mask is not None:
        ...
        reward = []
        for i, (kl_seg, action_len) in enumerate(zip(kl, num_actions)):
            kl_reward = -kl_coef * kl_seg  # Compute the KL penalty.
            last_reward = torch.full_like(kl_seg, r[i])  # Fill the whole sequence.
...
def masked_mean(tensor: torch.Tensor, mask: Optional[torch.Tensor], retrieve_mask: Optional[torch.Tensor], dim: int = None) -> torch.Tensor:
    if retrieve_mask is None:
        if mask is None:
