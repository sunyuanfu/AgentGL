...
def _get_dataset_categories(dataset: str, graph_data_dir: Optional[str]) -> List[str]:
    ...
    if not cats:
        default_root = os.environ.get("AGENTGL_NODE_DATA_ROOT", "node_data")
...
class PromptDataset(Dataset):
    ...
    def __init__(
        ...
        easy_to_hard = getattr(self.strategy.args, "curriculum_easy_to_hard", False)
        order = (0, 1, 2) if easy_to_hard else (2, 1, 0)
        # assemble prompts by difficulty order for curriculum-style training
        for rank in order:
