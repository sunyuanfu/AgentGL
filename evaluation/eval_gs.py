...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model",
        default=os.environ.get("AGENTGL_EVAL_MODEL", "Qwen/Qwen2.5-3B-Instruct"),
        help="Path to the model weights for vLLM",
    )
    parser.add_argument("--tokenizer", help="Optional tokenizer path; defaults to model path")
    parser.add_argument(
        "--eval-file",
        default=str(_REPO_ROOT / "data" / "eval_set" / "ogbn-arxiv.jsonl"),
        help="JSONL file containing evaluation samples (node_id & summary_en)",
    )
    parser.add_argument(
        "--output-file",
        default=str(_REPO_ROOT / "data" / "eval_results" / "ogbn-arxiv_agentgl_eval.jsonl"),
        help="Destination JSONL file under data/eval_results",
    )
    parser.add_argument(
        "--graph-data-dir",
        default=os.environ.get("AGENTGL_NODE_DATA_ROOT", str(_REPO_ROOT / "node_data")),
        help="Directory with graph artifacts (node_texts.json, category.json, etc.)",
    )
    parser.add_argument(
        "--graph-encoder-path",
        default=os.environ.get("AGENTGL_GRAPH_ENCODER_PATH", "all-roberta-large-v1"),
    ...
    # Keep only the first N samples when requested.
    parser.add_argument(
        "--head",
        type=int,
        default=1000,
