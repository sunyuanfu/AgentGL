...
ARXIV_JSONL=${LP_STAGE_ROOT}/ogbn-arxiv_stage1.jsonl
PRODUCTS_JSONL=${LP_STAGE_ROOT}/ogbn-products_stage1.jsonl

DATA_PATH="${ARXIV_JSONL},${PRODUCTS_JSONL}"
DATA_PROBS="0.5,0.5"
GRAPH_DATA_DIR="${DATA_PATH}"
LP_DIFFS_STAGE1="easy,medium"
...
SAVE_MODEL_NAME=qwen_lp_stage1_3b_grpo
GROUP_METHOD=normal
LOG_BASE=log
...
# Launch the Stage1 reward server beforehand:
#   python3 train/reward_server_lp.py --pair_data "${DATA_PATH}" --port ${PORT}
...
ray job submit --address="http://127.0.0.1:8267" \
   ...
   --lp_allowed_difficulties ${LP_DIFFS_STAGE1} \
   --ref_num_nodes 1 \
